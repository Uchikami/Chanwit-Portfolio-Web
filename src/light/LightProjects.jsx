...
function LightProjects() {
  return <section id="projects" className="prof-section prof-projects"><div className="prof-container">
    ...
    <div className="prof-project-list">{projects.map((project, index) => <article className="prof-project" key={project.title}><div className="prof-project-number">0{index + 1}</div><div><p className="prof-project-type">{project.type}</p><h3>{project.title}</h3><p>{project.description}</p><div className="prof-project-tags">{project.tags.map(tag => <span key={tag}>{tag}</span>)}</div></div><a className="prof-project-link" href="https://github.com/chanwit-loeyos" target="_blank" rel="noreferrer" aria-label={`View ${project.title} on GitHub`}><ArrowUpRight size={22} /></a></article>)}</div>
  </div></section>;
}
